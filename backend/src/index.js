...
app.post('/api/register/individual', async (req, res) => {
  ...
  try {
    // Check guesthouse availability (max 120 spaces)
    if (data.accommodation === 'guesthouse') {
      const result = await db.query(
        `SELECT COUNT(*) as count FROM individual_registrations WHERE accommodation = 'guesthouse'`
      );
      const guestHouseCount = parseInt(result.rows[0].count) || 0;
      
      if (guestHouseCount >= 120) {
        return res.status(400).json({ 
          success: false, 
          error: 'Guest House accommodation is fully booked. Please select Dormitory instead.' 
        });
      }
    }
